(()=>{
 const payload=window.NSWER_CHANTS;
 ...
 const card=item=>{
  ...
  const favorite=window.NSWER_FAVORITES?.button({type:'chants',id:item.slug||item.anchor||item.title,label:`${item.title||'掛け声'}をお気に入り`})||'';
 ...
 const render=()=>{
  ...
  listHost.innerHTML=items.length?items.map(card).join(''):'<div class="card notice"><strong>公開中のガイドはありません</strong><p>repositoryで「公開」にチェックすると表示されます。</p></div>';
  ...
  window.NSWER_FAVORITES?.bind(listHost);document.dispatchEvent(new CustomEvent('nswer:content-rendered'));
