(()=>{
  'use strict';
  const data=window.NSWER_MV||{};
  const items=(Array.isArray(data.items)?[...data.items]:[]).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||Number(b.order||0)-Number(a.order||0));
  ...
  function card(item){
    ...
    return `<a class="card mv-gallery-card" data-mv-year="${escapeHtml(item.year||'')}" data-type="${escapeHtml(item.type||'official')}" href="${escapeHtml(item.url||`https://www.youtube.com/watch?v=${item.videoId||''}`)}" rel="noopener noreferrer" target="_blank" id="${escapeHtml(item.anchor||`mv-${item.videoId||'item'}`)}">
      <div class="mv-gallery-thumb"><img alt="${escapeHtml(item.title||'NMIXX')} thumbnail" loading="lazy" src="${escapeHtml(item.thumbnail||`https://i.ytimg.com/vi/${item.videoId||''}/hqdefault.jpg`)}"/></div>
  ...
  function renderItems(){
    ...
    window.NSWER_IMAGE_OPTIMIZER?.refresh?.();
  }
