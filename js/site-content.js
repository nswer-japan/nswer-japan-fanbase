(()=>{
  ...
  function applyFooter(items){
    ...
    document.querySelectorAll('.site-footer').forEach(footer=>{
      ...
      if(note?.description&&noteElement){
        ...
        noteElement.append(year,document.createTextNode(` ${main?.heading||'NSWER JAPAN FB'}`));
      }
    });
  }
  ...
  function officialCard(item){
    const content=item.icon?`<span class="social-icon-wrap"><img src="${esc(item.icon)}" alt=""></span>`:`<span class="social-icon-wrap">${socialSvg(item.anchor)||esc(item.iconText||item.title.slice(0,2))}</span>`;
  ...
  async function renderOfficialLinks(){
    ...
    if(!root)return;
    try{
      let payload=window.NSWER_OFFICIAL_LINKS;
      if(!payload){
  ...
  async function load(){
    let payload=window.NSWER_HOMEPAGE;
    if(!payload){
    ...
    apply404(items);
    applyHome(items);
    window.dispatchEvent(new CustomEvent('nswer:site-content-ready',{detail:{count:items.length}}));
  }
