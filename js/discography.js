(()=>{
 ...
 let payload=window.NSWER_DISCOGRAPHY||null;
 let active='all';
 ...
 const releaseCard=release=>{
  ...
  const tracks=(Array.isArray(release.tracks)?release.tracks:[]).map((track,index)=>{
   ...
   const action=video?`<a aria-label="${escapeHtml(item.title)} YouTube" class="track-video" href="${escapeHtml(video)}" rel="noopener noreferrer" target="_blank">▶</a>`:(item.note?`<span class="track-note">${escapeHtml(item.note)}</span>`:'');
