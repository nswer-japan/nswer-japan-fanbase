(()=>{
  ...
  if(commonScript&&!document.querySelector('script[data-nswer-i18n]')){
    const languageScript=document.createElement('script');
    languageScript.src=new URL('./i18n.js',commonScript.src).href;
    languageScript.dataset.nswerI18n='true';
  ...
  const THEME_KEY='nswer-theme-mode';
  const LEGACY_THEME_KEY='nswer-theme';
  const FAVORITES_KEY='nswer-favorites-v1';
  ...
  const syncTheme=()=>{
    ...
    if(meta)meta.content=getComputedStyle(root).getPropertyValue('--bg').trim()||(resolved==='light'?'#f6f7ff':'#080b16');
  };
  const setTheme=mode=>{
    ...
    syncTheme();
    window.NSWER_ANALYTICS?.track?.('theme_change',{theme_mode:themeMode});
  };
  ...
  syncTheme();
  window.NSWER_THEME={get:()=>themeMode,set:setTheme};
  ...
  const writeFavorites=value=>{
    ...
    document.dispatchEvent(new CustomEvent('nswer:favorites-change',{detail:value}));
  };
  const favoriteApi={
    get:readFavorites,
    ...
    toggle(type,id,metadata={}){
      ...
      window.NSWER_ANALYTICS?.track?.(exists?'favorite_remove':'favorite_add',{favorite_type:type,favorite_id:key,...metadata});
      return !exists;
    },
  ...
  window.NSWER_FAVORITES=favoriteApi;
  ...
  const shareApi={
    async native({title=document.title,text='',url=location.href}={}){
      if(navigator.share){
        try{await navigator.share({title,text,url});window.NSWER_ANALYTICS?.track?.('share',{method:'native',content_title:title});return true;}catch(error){if(error?.name==='AbortError')return false;}
      }
      const result=await copyText(url);
      if(result)window.NSWER_ANALYTICS?.track?.('share',{method:'copy',content_title:title});
      return result;
    },
    x({title=document.title,text='',url=location.href}={}){
      ...
      window.NSWER_ANALYTICS?.track?.('share',{method:'x',content_title:title});
    },
    line({title=document.title,text='',url=location.href}={}){
      ...
      window.NSWER_ANALYTICS?.track?.('share',{method:'line',content_title:title});
    },
  ...
  const registerPwa=()=>{
    ...
    window.addEventListener('beforeinstallprompt',event=>{
      ...
      banner.innerHTML='<div><strong>ホーム画面へ追加</strong><span>NMIXXサイトをアプリのように開けます。</span></div><button type="button" data-pwa-install>追加</button><button type="button" class="pwa-dismiss" aria-label="閉じる">×</button>';
  ...
  document.addEventListener('nswer:content-rendered',()=>{favoriteApi.bind();shareApi.bind();});
})();
