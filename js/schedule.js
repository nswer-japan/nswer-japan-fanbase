(async () => {
  let schedulePayload = window.NSWER_SCHEDULE_PAYLOAD && typeof window.NSWER_SCHEDULE_PAYLOAD === "object"
    ? window.NSWER_SCHEDULE_PAYLOAD
    : { generatedAt: window.NSWER_SCHEDULE_GENERATED_AT || "", events: Array.isArray(window.NSWER_SCHEDULE) ? window.NSWER_SCHEDULE : [] };
  try {
  ...
  const recurringBirthdays = [
    { name: "LILY", month: 10, day: 17, description: "リリーの誕生日です。", link: "members.html#lily-profile" },
    { name: "HAEWON", month: 2, day: 25, description: "ヘウォンの誕生日です。", link: "members.html#haewon-profile" },
    { name: "SULLYOON", month: 1, day: 26, description: "ソリュンの誕生日です。", link: "members.html#sullyoon-profile" },
    { name: "BAE", month: 12, day: 28, description: "ベイの誕生日です。", link: "members.html#bae-profile" },
    { name: "JIWOO", month: 4, day: 13, description: "ジウの誕生日です。", link: "members.html#jiwoo-profile" },
    { name: "KYUJIN", month: 5, day: 26, description: "キュジンの誕生日です。", link: "members.html#kyujin-profile" },
  ];
  ...
  const cleanDescription = (value = "") => String(value)
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line && !/^原文\s*[:：]/i.test(line))
    .join("\n");
  const calendarFeedUrl = new URL("data/nswer-schedule.ics", document.baseURI).href;
  ...
  const eventDescription = (event) => [cleanDescription(event.description), absoluteEventLink(event) ? `関連リンク: ${absoluteEventLink(event)}` : "", `NSWER JAPAN FB: ${eventPageLink(event)}`].filter(Boolean).join("\n\n");
  const googleCalendarLink = (event) => {
    const params = new URLSearchParams({ action: "TEMPLATE", text: event.title || "NMIXX 予定", dates: calendarRange(event), details: eventDescription(event), ctz: "Asia/Tokyo" });
  ...
  const eventIcs = (event) => {
    const startValue = String(event.start || event.date || "");
    const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//NSWER JAPAN FB//Schedule//JA", "CALSCALE:GREGORIAN", "METHOD:PUBLISH", "BEGIN:VEVENT", `UID:${String(event.id || Date.now()).replace(/[^a-zA-Z0-9._-]/g, "-")}@nswerjapan.jp`, `DTSTAMP:${googleDate(new Date())}`, `SUMMARY:${escapeIcs(event.title || "NMIXX 予定")}`];
    if (startValue.includes("T")) {
      const start = new Date(startValue);
      let end = event.end ? new Date(event.end) : new Date(start.getTime() + 60 * 60 * 1000);
      if (Number.isNaN(end.getTime()) || end <= start) end = new Date(start.getTime() + 60 * 60 * 1000);
  ...
  const downloadEventIcs = (event) => {
    ...
    anchor.download = `${String(event.title || "NMIXX-event").replace(/[\\/:*?"<>|]/g, "-")}.ics`;
  ...
  const recurringEventsForYear = (year) => {
    ...
    if (year >= 2022) {
      ...
      events.push({
        ...
        title: anniversary > 0 ? `NMIXX デビュー${anniversary}周年` : "NMIXX デビュー記念日",
        date: `${year}-02-22`, start: `${year}-02-22`, end: "", category: "記録", type: "record",
        description: anniversary > 0 ? `2022年2月22日にデビューしたNMIXXのデビュー${anniversary}周年記念日です。` : "2022年2月22日にNMIXXがデビューしました。",
        link: "about.html", linkLabel: "NMIXXについて見る", image: "", recurringAnniversary: true,
      });
    }
    return events;
  };
  ...
  function renderEventList(events, year, month) {
    ...
    if (!filtered.length) {
      ...
      document.dispatchEvent(new CustomEvent("nswer:content-rendered"));
      return;
    }
    ...
    list.innerHTML = filtered.map((event) => {
      ...
      const shareText = `${String(event.start || event.date || "").slice(0, 10)} ${event.title || "NMIXX予定"}`;
  ...
  list.addEventListener("click", (event) => {
    ...
    if (selected) {
      ...
      window.NSWER_ANALYTICS?.track?.("calendar_add", { method: "ics", event_title: selected.title || "" });
    }
  });
  list.addEventListener("click", (event) => {
    ...
    window.NSWER_ANALYTICS?.track?.("calendar_add", { method: "google", event_title: selected?.title || "" });
  });
