(()=>{
  'use strict';
  const data=window.NSWER_VOTING_GUIDE;
  if(!data)return;
  ...
  const fav=(kind,item)=>window.NSWER_FAVORITES?.button({type:'voting',id:`${kind}:${item.slug||item.anchor||item.title}`,label:`${item.title||'投票ガイド'}をお気に入り`})||'';
  ...
  if(guideHost){
    guideHost.innerHTML=apps.map((item,index)=>{
      ...
      const stepHtml=steps.map(step=>`<article class="guide-step">${step.image?`<div class="guide-image">${image(step.image,`${item.title} ${step.title||'投票ガイド'}`)}</div>`:`<div class="guide-placeholder"><strong>${escapeHtml(step.title||'画像なし')}</strong><code>repositoryの「ガイド画像」へ追加できます</code></div>`}<div class="guide-caption"><b>${escapeHtml(step.title||'手順')}</b>${step.text?`<p>${escapeHtml(step.text)}</p>`:''}</div></article>`).join('');
  ...
  window.NSWER_FAVORITES?.bind(document);document.dispatchEvent(new CustomEvent('nswer:content-rendered'));
})();
