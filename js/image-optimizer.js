(()=>{
  ...
  if(!document.querySelector('script[data-nswer-i18n]')){
    const languageScript=document.createElement('script');
    languageScript.src=i18nUrl.href;
    languageScript.dataset.nswerI18n='true';
    document.head.appendChild(languageScript);
  }
  if(!document.querySelector('script[data-nswer-site-content]')){
    const contentScript=document.createElement('script');
    contentScript.src=siteContentUrl.href;
    contentScript.dataset.nswerSiteContent='true';
  ...
  async function start(){
    try{
      ...
      window.dispatchEvent(new CustomEvent('nswer:image-optimizer-ready',{detail:{count:Object.keys(data.images).length}}));
    }catch{
