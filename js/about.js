(()=>{
  'use strict';
  const about=window.NSWER_ABOUT;
  const membersPayload=window.NSWER_MEMBERS;
  ...
  if(page){
    text('aboutPageKicker',page.englishLabel||'ABOUT NMIXX');
  ...
  if(debut){
    ...
    if(image&&src){image.src=src;image.alt=`${debut.heading||debut.title}のNMIXXメンバー全体写真`;}
  }
  ...
  document.dispatchEvent(new CustomEvent('nswer:content-updated',{detail:{source:'about'}}));
})();
