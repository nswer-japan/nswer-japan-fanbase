(()=>{
 ...
 const trackSearch=(term,count)=>{clearTimeout(analyticsTimer);if(String(term).trim().length<2)return;analyticsTimer=setTimeout(()=>window.NSWER_ANALYTICS?.track?.('search',{search_term:String(term).trim(),result_count:Number(count)||0,search_category:activeFilter}),700);};
 ...
 const mvEntries=data=>(Array.isArray(data?.items)?data.items:[]).map((item,index)=>({
  ...
  summary:`NMIXXの${item.badge||item.kind||'公式映像'}です。${item.note||''}`,url:`mv.html#${item.anchor||`mv-${item.videoId||index}`}`,
 ...
 const aboutEntries=data=>(Array.isArray(data?.items)?data.items:[]).filter(item=>item?.type!=='ページ設定').map((item,index)=>({
  id:`about-${item.slug||index}`,category:'about',categoryLabel:item.englishLabel||item.type||'ABOUT NMIXX',title:item.heading||item.value||item.title||'NMIXXについて',
  summary:item.description||item.note||'',url:`about.html${item.anchor?`#${item.anchor}`:''}`,keywords:`${item.type||''} ${item.englishLabel||''} ${item.value||''} NMIXX グループ 紹介 コンセプト デビュー NSWER`,
  image:item.image||'',date:item.date||'',priority:77,
 }));


 const contactEntries=data=>(Array.isArray(data?.items)?data.items:[]).filter(item=>item?.type!=='ページ設定').map((item,index)=>({
  id:`contact-${item.slug||index}`,category:'contact',categoryLabel:item.englishLabel||item.type||'CONTACT',title:item.heading||item.title||'お問い合わせ',
  summary:item.description||'',url:`contact.html${item.anchor?`#${item.anchor}`:''}`,keywords:`${item.type||''} ${item.number||''} ${item.buttonLabel||''} お問い合わせ 訂正 情報提供 企画 広告 返信`,priority:65,
 }));
 ...
 const loadIndex=async()=>{
  const requests=[
   ...
   ['お問い合わせ',()=>fetchJson('data/contact.json')],
  ];
  ...
  settled.forEach((result,index)=>{
   ...
   if(index===12){entries=entries.filter(entry=>entry.category!=='contact'||entry.url==='contact.html');entries.push(...contactEntries(result.value));}
  });
  const seen=new Set();
