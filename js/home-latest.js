(()=>{
  ...
  const recurringDefinitions=[
    {name:'LILY',month:10,day:17,type:'birthday',category:'Birthday',description:'リリーの誕生日です。'},
    {name:'HAEWON',month:2,day:25,type:'birthday',category:'Birthday',description:'ヘウォンの誕生日です。'},
    {name:'SULLYOON',month:1,day:26,type:'birthday',category:'Birthday',description:'ソリュンの誕生日です。'},
    {name:'BAE',month:12,day:28,type:'birthday',category:'Birthday',description:'ベイの誕生日です。'},
    {name:'JIWOO',month:4,day:13,type:'birthday',category:'Birthday',description:'ジウの誕生日です。'},
    {name:'KYUJIN',month:5,day:26,type:'birthday',category:'Birthday',description:'キュジンの誕生日です。'},
  ];
  const recurringEvents=years=>{
    ...
    years.forEach(year=>{
      ...
      if(year>=2022){
        ...
        const date=`${year}-02-22`;
        events.push({id:`auto-debut-anniversary-${year}`,title:anniversary>0?`NMIXX デビュー${anniversary}周年`:'NMIXX デビュー記念日',date,start:date,end:'',category:'記録',type:'record',description:`2022年2月22日にデビューしたNMIXXの${anniversary>0?`${anniversary}周年`:'デビュー'}記念日です。`});
      }
    });
    return events;
  };
  ...
  const renderNews=payload=>{
    const source=Array.isArray(payload?.news)?payload.news:(Array.isArray(window.NSWER_NEWS)?window.NSWER_NEWS:[]);
  ...
  Promise.allSettled(jobs.map(job=>fetchJson(job.path))).then(results=>{
    ...
    results.forEach((result,index)=>{
      ...
      if(result.status==='fulfilled'){
        ...
        if(job.key==='news'&&Array.isArray(window.NSWER_NEWS)){
          renderNews({news:window.NSWER_NEWS});success+=1;
