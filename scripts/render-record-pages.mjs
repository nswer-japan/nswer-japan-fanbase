...
const hanteo = Array.isArray(payload.hanteoRecords) ? [...payload.hanteoRecords] : [];

wins.sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")) || Number(a.order ?? 9999) - Number(b.order ?? 9999));
hanteo.sort((a,b)=>String(a.releaseDate||"9999-99-99").localeCompare(String(b.releaseDate||"9999-99-99")));
...
const winList = wins.length ? `<div class="wins-list">${[...wins].reverse().map((item, index) => {
  ...
  const image = safeImage(item.image, "assets/group/nmixx-group.jpg");
...
const bestFirstWeek = hanteo.reduce((best,item)=>Math.max(best,Number(item.firstWeek)||0),0);
const latestHanteo = hanteo.at(-1);
const hanteoOverview = `<div class="record-overview" aria-label="Hanteo初動記録の概要"><article class="card record-stat"><small>ALBUMS</small><strong>${hanteo.length}</strong><span>掲載作品数</span></article><article class="card record-stat"><small>BEST FIRST WEEK</small><strong>${number(bestFirstWeek)}</strong><span>初動最高枚数</span></article><article class="card record-stat"><small>LATEST</small><strong>${esc(latestHanteo?.album||"—")}</strong><span>最新掲載作品</span></article></div>`;
const hanteoList = hanteo.length ? `<div class="melon-record-list">${hanteo.map((item,index)=>`<article class="card melon-record-card"><div class="melon-record-index">${String(index+1).padStart(2,"0")}</div><div class="melon-record-art"><img src="${esc(safeImage(item.image,"assets/group/nmixx-group.jpg"))}" alt="${esc(item.album||"")}" loading="lazy"></div><div class="melon-record-copy"><small>RELEASE ${esc(dateJa(item.releaseDate))}</small><h2>${esc(item.album||"")}</h2><p>${esc(item.type||"")} / Title: ${esc(item.titleTrack||"—")}</p></div><dl class="melon-rank-grid"><div><dt>初日売上</dt><dd>${number(item.firstDay)}枚</dd></div><div><dt>初動売上</dt><dd>${number(item.firstWeek)}枚</dd></div></dl></article>`).join("\n")}</div>` : `<div class="card record-empty">公開中のHanteo記録はまだありません。</div>`;
await replaceBlock("hanteo-records.html", "HANTEO-OVERVIEW", hanteoOverview);
await replaceBlock("hanteo-records.html", "HANTEO-LIST", hanteoList);

console.log(`記録ページを生成しました（音楽番組1位 ${wins.length}件 / Melon ${melon.length}件 / Hanteo ${hanteo.length}件）。`);
