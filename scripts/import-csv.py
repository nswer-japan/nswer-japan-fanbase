...
PLACEHOLDERS = {"example-news", "example-event", "Example Song", "タイトル", "予定タイトル"}
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] not in CONFIG:
        raise SystemExit("使い方: python scripts/import-csv.py news|schedule|wins")
    kind = sys.argv[1]
    csv_rel, json_rel, array_key = CONFIG[kind]
    rows = clean_rows(ROOT / csv_rel)
    normalized = {"news": normalize_news, "schedule": normalize_schedule, "wins": normalize_wins}[kind](rows)
